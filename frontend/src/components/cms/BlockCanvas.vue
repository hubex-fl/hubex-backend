...
function renderBlockHtml(block: CmsBlock): string {
  ...
  switch (block.type) {
    case "heading": {
      ...
      return `<${level} style="font-size:${sizes[level]}px;font-weight:700;color:#F5F5F5;margin:24px 0 12px;text-align:${align};">${escapeHtml(p.text || "Heading")}</${level}>`;
    }
    case "text":
      return `<div style="color:#A1A1AA;line-height:1.7;font-size:16px;margin:12px 0;">${p.content || "<em>Empty text block</em>"}</div>`;
    case "image": {
      const src = escapeAttr(p.src || "");
      if (!src) return `<div style="padding:24px;text-align:center;color:#71717A;border:1px dashed rgba(255,255,255,0.15);border-radius:8px;">Image — no src set</div>`;
    ...
    case "hero": {
      const title = escapeHtml(p.title || "Hero Title");
    ...
    case "feature_grid": {
      ...
      const cards = items
        ...
        .join("");
      return `<section style="display:grid;grid-template-columns:repeat(${cols},1fr);gap:24px;max-width:1200px;margin:48px auto;padding:0 24px;">${cards || '<div style="color:#71717A;padding:24px;text-align:center;grid-column:1/-1;">No items yet</div>'}</section>`;
    }
    case "cta": {
      const title = escapeHtml(p.title || "Call to Action");
      const desc = escapeHtml(p.description || "");
      const btn = escapeHtml(p.button_text || "Click");
    ...
    case "spacer": {
      ...
      return `<div style="height:${h}px;background:rgba(255,255,255,0.02);border:1px dashed rgba(255,255,255,0.06);text-align:center;color:#52525B;font-size:11px;display:flex;align-items:center;justify-content:center;">Spacer (${h}px)</div>`;
    }
    case "divider": {
    ...
    case "columns": {
      ...
      const cols = items
        .map(
          (it) =>
            `<div style="flex:1;min-width:0;padding:16px;border:1px dashed rgba(255,255,255,0.1);border-radius:6px;color:#A1A1AA;">${typeof it?.content === "string" ? it.content : "<em>Column</em>"}</div>`,
        )
        .join("");
    ...
    case "html":
      return p.content || `<div style="color:#71717A;padding:12px;font-style:italic;">Empty HTML block</div>`;
    case "video": {
      const url = escapeAttr(p.url || "");
      if (!url) return `<div style="padding:24px;text-align:center;color:#71717A;border:1px dashed rgba(255,255,255,0.15);border-radius:8px;">Video — no URL set</div>`;
    ...
    case "quote": {
      const text = escapeHtml(p.text || "Quote text");
    ...
    case "stats": {
      ...
      return `<section style="display:grid;grid-template-columns:repeat(${cols},1fr);gap:24px;max-width:1000px;margin:48px auto;padding:0 24px;">${cells || '<div style="color:#71717A;padding:24px;text-align:center;grid-column:1/-1;">No stats yet</div>'}</section>`;
    }
    case "button": {
    ...
    case "list": {
      ...
      return `<${tag} style="color:#A1A1AA;max-width:800px;margin:16px auto;padding:0 24px 0 48px;line-height:1.8;">${lis || "<li><em>No items</em></li>"}</${tag}>`;
    }
    ...
    case "dashboard_embed": {
      ...
      if (!dashId) {
        return `<div style="padding:32px;text-align:center;color:#71717A;border:1px dashed rgba(245,166,35,0.3);border-radius:12px;margin:16px auto;max-width:960px;"><div style="font-size:24px;margin-bottom:8px;">📊</div>Dashboard embed — set a dashboard ID in the properties panel.</div>`;
      }
      return `<div style="margin:16px auto;max-width:1200px;padding:0 24px;"><div style="height:${h}px;background:#111110;border:1px solid rgba(255,255,255,0.08);border-radius:12px;display:flex;align-items:center;justify-content:center;color:#71717A;">📊 Live dashboard #${dashId} (renders at runtime)</div></div>`;
    }
    case "variable_value": {
      const label = escapeHtml(p.label || p.key || "variable");
    ...
    case "device_card": {
      ...
      return `<div style="max-width:520px;margin:16px auto;padding:20px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:12px;"><div style="color:#F5F5F5;font-weight:600;">🖥 Device: ${uid || '(not set)'}</div><div style="color:#71717A;font-family:'IBM Plex Mono',monospace;font-size:12px;margin-top:4px;">loads at runtime</div></div>`;
    }
    case "device_list": {
      const cols = Math.max(1, Math.min(6, Number(p.columns) || 3));
      const filter = escapeHtml(p.filter || "all");
      return `<div style="max-width:1200px;margin:16px auto;padding:0 24px;"><div style="color:#A1A1AA;font-size:11px;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:8px;">Devices (${filter})</div><div style="display:grid;grid-template-columns:repeat(${cols},1fr);gap:16px;"><div style="grid-column:1/-1;padding:24px;text-align:center;color:#71717A;border:1px dashed rgba(255,255,255,0.08);border-radius:8px;">Device grid — loads at runtime</div></div></div>`;
    }
    case "tour_trigger": {
      const text = escapeHtml(p.button_text || "Take a tour");
    ...
    case "alert_banner": {
      ...
      return `<div style="max-width:960px;margin:16px auto;padding:14px 18px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:10px;color:#A1A1AA;">⚠ Alert banner (${severity}) — live at runtime</div>`;
    }
    case "metric_counter": {
      const label = escapeHtml(p.label || p.metric || "metric");
    ...
    case "automation_status": {
      ...
      return `<div style="max-width:520px;margin:16px auto;padding:20px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:12px;"><div style="color:#F5F5F5;font-weight:600;">⚙ Automation ${id ? `#${id}` : '(not set)'}</div><div style="color:#71717A;font-size:13px;margin-top:4px;">loads at runtime</div></div>`;
    }
    default:
      return `<div style="padding:16px;background:rgba(255,255,255,0.03);border:1px dashed rgba(255,255,255,0.1);border-radius:6px;color:#A1A1AA;">Block: ${block.type}</div>`;
  }
}
  ...
  <div class="block-canvas">
    <div v-if="blocks.length === 0" class="empty">
      <div class="empty-inner">
        ...
        <div class="empty-sub">
          Pick a block from the left to get started.
        </div>
      </div>
    </div>
    <div
