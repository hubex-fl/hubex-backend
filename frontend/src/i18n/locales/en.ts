export default {
  ...
  events: {
    ...
    start: 'Start',
    stream: 'Stream',
    streamTooltip: 'An event stream is a named channel that groups related events.',
    streamTooltipItems: [
      '"system" is the default stream for platform-wide events',
      'Device-specific streams are named "device:<uid>" (auto-created when a device sends telemetry)',
      'Select a stream to filter which events you see below',
    ],
    ...
    traceIdTooltip: 'Filter events by Trace ID — a unique identifier that links related events.',
    traceIdTooltipItems: [
      'Trace IDs appear automatically when an event triggers downstream actions',
      'Type a trace ID directly or select one from the dropdown (shows IDs from current events)',
      'Useful to follow a data point from device ingestion through automation execution',
    ],
  ...
  pages: {
    ...
    entities: {
      ...
      sectionHierarchy: 'Hierarchy (optional)',
      sectionLocation: 'Location (optional)',
      hierarchyExplainer: 'Assign a parent to build nested structures like Site → Building → Floor → Room. This lets you scope dashboards, automations, and permissions to specific parts of your organization.',
      locationGpsHint: 'Enter an address or GPS coordinates. A map picker will be available in a future update.',
