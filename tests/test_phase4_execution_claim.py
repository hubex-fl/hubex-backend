...
@pytest.mark.asyncio
async def test_execution_claim_capability_enforced(monkeypatch):
    ...
    res = await client.post(f"/api/v1/executions/runs/{run.id}/claim", json={"worker_id": "w1"})
...
@pytest.mark.asyncio
async def test_execution_claim_happy_path_and_conflicts(monkeypatch):
    monkeypatch.setenv("HUBEX_CAPS_ENFORCE", "1")
    CAPABILITY_MAP[("POST", "/api/v1/executions/runs/{run_id}/claim")] = ["executions.write"]
    ...
    assert res2.json()["claimed_by"] == "worker-a"

    res3 = await client.post(
        f"/api/v1/executions/runs/{run.id}/claim",
        json={"worker_id": "worker-b", "lease_seconds": 120},
        headers={"Authorization": f"Bearer {token_ok}"},
    )
...
@pytest.mark.asyncio
async def test_execution_claim_expired_lease(monkeypatch):
    monkeypatch.setenv("HUBEX_CAPS_ENFORCE", "1")
    CAPABILITY_MAP[("POST", "/api/v1/executions/runs/{run_id}/claim")] = ["executions.write"]
