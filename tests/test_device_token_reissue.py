...
@pytest.mark.asyncio
async def test_device_token_reissue_owner_success_and_audit(monkeypatch):
    monkeypatch.setenv("HUBEX_CAPS_ENFORCE", "1")
    CAPABILITY_MAP[("POST", "/api/v1/devices/{device_id}/token/reissue")] = [
        "devices.token.reissue"
    ]

    engine, Session = await _mk_session()
    async with Session() as db:
        user = User(id=1, email="owner@example.com", password_hash="x", caps=["devices.token.reissue"])
        device = Device(device_uid="dev-1", owner_user_id=1, is_claimed=True)
        db.add_all([user, device])
        await db.commit()
        await db.refresh(device)
        db.add(DeviceToken(device_id=device.id, token_hash="old", is_active=True))
        await db.commit()
    ...
    res = await client.post(
        f"/api/v1/devices/{device.id}/token/reissue",
        json={"reason": "lost device token"},
        headers={"Authorization": f"Bearer {_token('1', ['devices.token.reissue'])}"},
    )
...
@pytest.mark.asyncio
async def test_device_token_reissue_non_owner_forbidden(monkeypatch):
    monkeypatch.setenv("HUBEX_CAPS_ENFORCE", "1")
    CAPABILITY_MAP[("POST", "/api/v1/devices/{device_id}/token/reissue")] = [
        "devices.token.reissue"
    ]
    ...
    res = await client.post(
        f"/api/v1/devices/{device.id}/token/reissue",
        json={"reason": "lost"},
        headers={"Authorization": f"Bearer {_token('2', ['devices.token.reissue'])}"},
    )
...
@pytest.mark.asyncio
async def test_device_token_reissue_unknown_device(monkeypatch):
    monkeypatch.setenv("HUBEX_CAPS_ENFORCE", "1")
    CAPABILITY_MAP[("POST", "/api/v1/devices/{device_id}/token/reissue")] = [
        "devices.token.reissue"
    ]
    ...
    res = await client.post(
        "/api/v1/devices/999/token/reissue",
        json={"reason": "lost"},
        headers={"Authorization": f"Bearer {_token('1', ['devices.token.reissue'])}"},
    )
    assert res.status_code == 404
    assert res.json().get("detail", {}).get("code") == "DEVICE_NOT_FOUND"
...
@pytest.mark.asyncio
async def test_device_token_reissue_unclaimed_device(monkeypatch):
    monkeypatch.setenv("HUBEX_CAPS_ENFORCE", "1")
    CAPABILITY_MAP[("POST", "/api/v1/devices/{device_id}/token/reissue")] = [
        "devices.token.reissue"
    ]
    ...
    async with Session() as db:
        user = User(id=1, email="owner@example.com", password_hash="x", caps=[])
        device = Device(device_uid="dev-3", owner_user_id=None, is_claimed=False)
        db.add_all([user, device])
        await db.commit()
        await db.refresh(device)
    ...
    assert res.json().get("detail", {}).get("code") == "DEVICE_NOT_CLAIMED"
