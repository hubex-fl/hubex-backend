...
@pytest.mark.asyncio
async def test_execution_run_read_by_id_capability_enforced(monkeypatch):
    monkeypatch.setenv("HUBEX_CAPS_ENFORCE", "1")
    CAPABILITY_MAP[("GET", "/api/v1/executions/runs/{run_id}")] = ["executions.read"]
    ...
    res = await client.get("/api/v1/executions/runs/1")
...
@pytest.mark.asyncio
async def test_execution_run_read_by_id_happy_path(monkeypatch):
    monkeypatch.setenv("HUBEX_CAPS_ENFORCE", "1")
    CAPABILITY_MAP[("GET", "/api/v1/executions/runs/{run_id}")] = ["executions.read"]
    ...
    res = await client.get(
        f"/api/v1/executions/runs/{run.id}",
