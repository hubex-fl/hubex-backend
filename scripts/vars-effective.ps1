...
$IncludeSecrets = $env:HUBEX_INCLUDE_SECRETS
...
if ($IncludeSecrets) { $query = "$query&includeSecrets=true" }
$resp = Invoke-Api "GET" "$Base/api/v1/variables/effective?$query" $null @{ "Authorization" = "Bearer $Token" }
